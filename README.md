В проекте находятся тесты для описанных ниже сценариев, расположенные в соответствующих файлах. 

## Структура проекта
Sprint_5/
├── Tests/
│   ├── test_creation_ad.py	    # Тесты создания объявления
│   ├── test_login.py			# Тесты авторизации пользователя
│   ├── test_logout.py			# Тесты выхода из аккаунта
│   └── test_registration.py	# Тесты регистрации нового пользователя
├── conftest.py					# Конфигурация pytest: фикстуры
├── locators.py					# Локаторы UI‑элементов
├── README.md					# Документация по тестовому набору
└── test_data.py				# Тестовые данные и утилиты для их генерации

*Хочу обратить внимание* на вторые варианты сценариев для Logout и создания объявления, их я добавила от себя, чтобы пройти путь не только через авторизацию, но и через регистрацию. 

Дело в том, что *авторизация сломана* (при верных данных выдает ошибку), поэтому тесты с использованием авторизации для данного задания заведомо провальны. Прикладываю скринкаст проверки авторизации в ручном режиме (зарегистрировала юзера, вышла, попыталась авторизоваться) [скринкаст](https://disk.yandex.ru/i/zcUxIvzJQvo7cg)

**ТЕСТЫ РЕГИСТРАЦИИ (файл test_registration)**

__Регистрация пользователя:__
•	Нажать кнопку «Вход и регистрация».
•	Нажать кнопку «Нет аккаунта».
•	Заполнить все поля формы регистрации и нажать кнопку «Создать аккаунт».
•	Проверить: произошёл переход на главную страницу, в правом верхнем углу около кнопки «Разместить объявление» отображается аватар пользователя и имя User.

__Регистрация пользователя c email не по маске *******@*****.**:__
•	Нажать кнопку «Вход и регистрация».
•	Нажать кнопку «Нет аккаунта».
•	Заполнить поле Email формы регистрации и нажать кнопку «Создать аккаунт».
•	Проверить: поля Email, «Пароль», «Повторите пароль» выделены красным, под полем Email отображается сообщение «Ошибка».

__Регистрация уже существующего пользователя:__ 
•	Нажать кнопку «Вход и регистрация».
•	Нажать кнопку «Нет аккаунта».
•	Заполнить все поля формы регистрации данными уже существующего в системе пользователя и нажать кнопку «Создать аккаунт».
•	Проверить: поля Email, «Пароль», «Повторите пароль» выделены красным, под полем Email отображается сообщение «Ошибка».

**ТЕСТЫ АВТОРИЗАЦИИ (файлы test_login и test_logout)**

__Login пользователя:__ 
•	Нажать кнопку «Вход и регистрация».
•	Заполнить все поля формы авторизации и нажать кнопку «Войти».
•	Проверить: произошёл переход на главную страницу, в правом верхнем углу около кнопки «Разместить объявление» отображается аватар пользователя и имя User.

__Logout пользователя:__
•	Авторизоваться под заранее созданным пользователем.
•	Нажать кнопку «Выйти».
•	Проверить: аватар пользователя и имя User больше не отображается в правом верхнем углу около кнопки «Разместить объявление», там теперь отображается кнопка «Вход и регистрация».

**ТЕСТЫ СОЗДАНИЯ ОБЪЯВЛЕНИЯ (файл test_creation_ad)**

__Создание объявления неавторизованным пользователем:__
•	Нажать кнопку «Разместить объявление».
•	Проверить: отображается модальное окно с заголовком «Чтобы разместить объявление, авторизуйтесь».

__Создание объявления авторизованным пользователем:__
•	Авторизоваться под заранее созданным пользователем.
•	Заполнить все поля формы: «Название», «Описание товара», «Стоимость» — стоимость должна быть указана в числовом формате.
•	Выбрать из Dropdown «Категорию» и «Город».
•	Выбрать RabioButton «Состояние товара».
•	Нажать кнопку «Опубликовать».
•	Перейти в профиль пользователя.
•	Проверить: в блоке «Мои объявления» отображается созданное объявление.
